...
生成: 2026-09-01 03:43Z / 対象: 2026-08-26 → 2026-09-01 / 観測 7 回 (1 日 1 回、シンガポールの VPS 1 台から、各回 200 件サンプル)。
文章は定型で、数字はすべて `data/observatory.csv` と `data/market_desk.csv` から機械的に計算したものです。AI による解釈は含みません。

## lobby の状態 (鍵量産 = farm の指標)
- 投稿速度: 平均 36.8 (min 22.4 / max 50.6) msg/s
...
## ルームの増減
- 一覧上のルーム総数: 8327 → 56376 (+48049)
- 新規ルーム作成: 平均 404 (min 59 / max 828) /時

## 市場の温度計 (Binance USDT 建て無期限、メジャー 13 銘柄除外)
- 負乖離シェア: 平均 57% (min 51% / max 65%) (30 日平均の最新値 57%、レジームゲート閾値 80%)
- 資金調達率: 過熱 (+0.05%/8h 以上) 銘柄数 平均 7 (min 3 / max 11) / マイナス銘柄数 平均 57 (min 45 / max 86)
- 清算 (24h、USDT 建てのみ、ストリーム標本): ロング清算 平均 $101M (min $21M / max $179M) / ショート清算 平均 $94M (min $27M / max $159M)
...
Generated 2026-09-01 03:43Z / window 2026-08-26 → 2026-09-01 / 7 daily probes from one VPS (Singapore), 200-message samples.
...
- message rate: mean 36.8 (min 22.4 / max 50.6) msg/s
...
## Rooms
- listed room count: 8327 → 56376 (+48049)
- new rooms created: mean 404 (min 59 / max 828) per hour
...
- funding: hot (>= +0.05%/8h) symbols mean 7 (min 3 / max 11) / negative symbols mean 57 (min 45 / max 86)
- liquidations (24h, USDT-perps, sampled stream): long-liq mean $101M (min $21M / max $179M) / short-liq mean $94M (min $27M / max $159M)

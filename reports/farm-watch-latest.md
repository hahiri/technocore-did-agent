...
生成: 2026-09-02 03:47Z / 対象: 2026-08-26 → 2026-09-02 / 観測 8 回 (1 日 1 回、シンガポールの VPS 1 台から、各回 200 件サンプル)。
文章は定型で、数字はすべて `data/observatory.csv` と `data/market_desk.csv` から機械的に計算したものです。AI による解釈は含みません。

## lobby の状態 (鍵量産 = farm の指標)
- 投稿速度: 平均 38.4 (min 22.4 / max 50.6) msg/s
- 200 件中の別々の鍵の割合: 平均 99% (100% に近いほど「1 鍵 1 投稿」の量産型)
- 定型文の重複率: 平均 23% (min 4% / max 64%)
- 履歴が流れるまでの推定時間: 平均 20 (min 11 / max 31) 分 (10 MiB のリングが埋まる速さ)

## ルームの増減
- 一覧上のルーム総数: 8327 → 51905 (+43578)
- 新規ルーム作成: 平均 398 (min 59 / max 828) /時

## 市場の温度計 (Binance USDT 建て無期限、メジャー 13 銘柄除外)
- 負乖離シェア: 平均 57% (min 51% / max 65%) (30 日平均の最新値 57%、レジームゲート閾値 80%)
- 資金調達率: 過熱 (+0.05%/8h 以上) 銘柄数 平均 8 (min 3 / max 14) / マイナス銘柄数 平均 57 (min 45 / max 86)
- 清算 (24h、USDT 建てのみ、ストリーム標本): ロング清算 平均 $109M (min $21M / max $179M) / ショート清算 平均 $90M (min $27M / max $159M)
...
Generated 2026-09-02 03:47Z / window 2026-08-26 → 2026-09-02 / 8 daily probes from one VPS (Singapore), 200-message samples.
...
- message rate: mean 38.4 (min 22.4 / max 50.6) msg/s
- distinct keys per 200 messages: mean 99% (close to 100% = one-key-one-post farms)
- duplicated canned lines: mean 23% (min 4% / max 64%)
- estimated ring retention: mean 20 (min 11 / max 31) min

## Rooms
- listed room count: 8327 → 51905 (+43578)
- new rooms created: mean 398 (min 59 / max 828) per hour
...
- funding: hot (>= +0.05%/8h) symbols mean 8 (min 3 / max 14) / negative symbols mean 57 (min 45 / max 86)
- liquidations (24h, USDT-perps, sampled stream): long-liq mean $109M (min $21M / max $179M) / short-liq mean $90M (min $27M / max $159M)
